...
transformed_matrix = array(NA, c(1, totalIter, 2))
MI_estimates = array(NA, c(1, totalIter, 2))
ipw_estimates = array(NA, c(1, totalIter, 2))
...
{
  for (kk in 1:length(zeta))
  {
    writeLines(paste("\nn: ", n, ", zeta: ", zeta[kk], " , rho: ", rho[kk], "\n", sep=""))
                    ...
                    .init=list(list(), list(), list(), list(), list(), list()), .options.snow = opts) %dopar%
    {
      ...
      transformed[1] = log(EM_output$gamma_est[3]/EM_output$gamma_est[1]) / 2 + (log(EM_output$gamma_est[4]/EM_output$gamma_est[1]) - log(EM_output$gamma_est[2]/EM_output$gamma_est[1])) / 2
      
      ########################## multiple imputation
      
      M = 50
      stacked_impute = doImputation(X_obs, Y, delta, R, cc_empirical_prob, EM_output, zeta[kk], M)
      
      impute_output = coxph(Surv(Y, delta) ~ X1+X2, data = stacked_impute)$coefficients
      
      ############################ ipw
      
      lambda = get_IPW_weights(X_cc, Y_cc, delta_cc, eta_10_est, eta_01_est, rho[kk])
    ...
    transformed_matrix[n_iter, , 1] = unlist(output[[1]])
    transformed_matrix[n_iter, , 2] = unlist(output[[2]])
    MI_estimates[n_iter, , 1] = unlist(output[[5]])
    MI_estimates[n_iter, , 2] = unlist(output[[6]])
    ipw_estimates[n_iter, , 1] = unlist(output[[7]])
    ipw_estimates[n_iter, , 2] = unlist(output[[8]])
    
    save(MI_estimates, file=paste("survival_sim_MI_sensitivity_",zeta[kk],".RData", sep=""))
    save(ipw_estimates, file=paste("survival_sim_ipw_sensitivity_",rho[kk],".RData", sep=""))
